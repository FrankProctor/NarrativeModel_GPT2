...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer,
        ...
        with open(file_path, encoding="utf-8") as f:
            lines = [
                line for line in f.read().splitlines() if (
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs, labels = mask_tokens(
                batch, tokenizer, args) if args.mlm else (
                batch, batch)
...
def evaluate(args, model: PreTrainedModel,
             tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
...
def main():
    ...
    if args.model_type in ["bert", "roberta",
                           "distilbert", "camembert"] and not args.mlm:
        raise ValueError(
