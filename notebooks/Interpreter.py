...
class Interpreter(nn.Module):
    ...
    def __init__(self, x, Phi, scale=0.5, rate=0.1,
                 regularization=None, words=None):
        ...
        if self.words is not None:
            assert self.s == len(
                words), 'the length of x should be of the same with the lengh of words'
    ...
    def forward(self):
        ...
        x = self.x + 0.    # S * D
        x_tilde = x + ratios * \
            torch.randn(self.s, self.d).to(x.device) * self.scale  # S * D
    ...
    def optimize(self, iteration=5000, lr=0.01, show_progress=False):
        ...
        for _ in func(range(iteration)):
            ...
            if minLoss is None or minLoss > loss:
                state_dict = {
                    k: self.state_dict()[k] +
                    0. for k in self.state_dict().keys()}
                minLoss = loss
        self.eval()
