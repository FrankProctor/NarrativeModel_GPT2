...
requirejs(["jquery", "d3"], function($, d3) {
  ...
  function render() {
    ...
    config.thumbnailHeight =
      Math.max(config.leftText.length, config.rightText.length) *
        config.thumbnailBoxHeight +
      2 * THUMBNAIL_PADDING;
  ...
  function renderThumbnail(layerIndex, headIndex) {
    ...
    renderThumbnailAttn(
      x,
      y,
      config.attn[layerIndex][headIndex],
      layerIndex,
      headIndex
    );
  }
  ...
  function renderDetail(att, layerIndex, headIndex) {
    var xOffset = 0.8 * config.thumbnailWidth;
    var maxX = DIV_WIDTH;
  ...
  function highlightSelection(index) {
    ...
    config.svg.selectAll(".attn-line-group").style("opacity", function(d, i) {
      return i == index ? 1.0 : 0.0;
    });
  }
  ...
  function renderThumbnailAttn(x, y, att, layerIndex, headIndex) {
    ...
    var attnBackground = attnContainer
      .append("rect")
    ...
    attnContainer
      ...
      .attr("y1", function(d) {
        ...
        return y1 + (sourceIndex + 0.5) * config.thumbnailBoxHeight;
      })
      .attr("x2", x2)
      .attr("y2", function(d, targetIndex) {
        return y1 + (targetIndex + 0.5) * config.thumbnailBoxHeight;
      })
    ...
    clickRegion.on("click", function(d, index) {
      ...
      attnBackgroundOther.attr("fill", "white");
      ...
      if (
        ...
        config.detail_layer = layerIndex;
        config.detail_head = headIndex;
        attnBackground.attr("fill", "white");
        attnBackground.attr("stroke-opacity", 0.8);
      } else {
