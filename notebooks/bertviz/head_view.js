...
requirejs(["jquery", "d3"], function($, d3) {
  ...
  console.log(d3);
  const HEAD_COLORS = x =>
    d3.interpolateRainbow((x / (config.nHeads - 1)) * 0.8); //d3.scaleOrdinal(d3.schemeCategory10);
  initialize();
  ...
  function renderText(svg, text, isLeft, attData, leftPos) {
    ...
    var tokenContainer = textContainer
      .append("g")
      .selectAll("g")
      .data(text)
      .enter()
    ...
    tokenContainer.on("mouseover", function(d, index) {
      textContainer.selectAll(".background").style("opacity", function(d, i) {
      ...
      svg
        .selectAll(".lineHeads") // To get the nesting to work.
        .selectAll(".attLines")
  ...
  function boxOffsets(i) {
    var numHeadsAbove = config.headVis.reduce(function(acc, val, cur) {
